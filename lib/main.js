...
function ztest2( x, y, sigmax, sigmay, options ) {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '1M18R', x ) );
	}
	if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
		throw new TypeError( format( '1M19k', y ) );
	}
	if ( !isPositiveNumber( sigmax ) ) {
		throw new TypeError( format( '1M1A7', sigmax ) );
	}
	if ( !isPositiveNumber( sigmay ) ) {
		throw new TypeError( format( '1M1A8', sigmay ) );
	}
	opts = {};
	if ( options ) {
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '1M14S', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	out = {};
