...
function ztest2( x, y, sigmax, sigmay, options ) {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '1M18R,GO', x ) );
	}
	if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
		throw new TypeError( format( '1M19k,GP', y ) );
	}
	if ( !isPositiveNumber( sigmax ) ) {
		throw new TypeError( format( '1M1A7,Gq', sigmax ) );
	}
	if ( !isPositiveNumber( sigmay ) ) {
		throw new TypeError( format( '1M1A8,Gr', sigmay ) );
	}
	opts = {};
	if ( options ) {
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '1M12X,3g,4S,GD,Gg,Jm', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	out = {};
